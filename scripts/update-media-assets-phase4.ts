...
async function updateMediaAssetsPhase4() {
	...
	const allCollections = await requestJson(`${baseUrl}/api/collections?page=1&perPage=250`, {
	...
	for (const collectionName of PHASE4_COLLECTIONS) {
	...
	console.log('Phase 4 sponsor deliverables collections are ready.');
}

updateMediaAssetsPhase4().catch((error) => {
