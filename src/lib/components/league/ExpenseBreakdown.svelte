<script lang="ts">
	...
	let totalExpenses = $derived(
		Object.values(expenseBreakdown).reduce((sum, val) => sum + val, 0)
	);
...
<Card class="p-6">
	<div class="flex items-center gap-3 mb-6">
		<div class="p-2 bg-red-100 dark:bg-red-900 rounded-lg">
			<Receipt class="size-5 text-red-600 dark:text-red-400" />
		</div>
		<div>
			<h3 class="text-lg font-semibold">Expense Breakdown</h3>
			<p class="text-sm text-muted-foreground">Costs by category</p>
		</div>
	</div>
	
	{#if sortedExpenses.length > 0}
		<div class="space-y-4">
			{#each sortedExpenses as [category, amount], index}
