...
<Sheet.Root {open} onOpenChange={handleOpenChange}>
	<Sheet.Content side="left" class="w-full sm:max-w-2xl overflow-y-auto bg-slate-900 text-white p-6 h-full">
		...
		<form onsubmit={handleSubmit} class="space-y-6">
			...
			<!-- Budget (calculated from projects, read-only display + manual override) -->
			<div class="space-y-2">
				...
				<Input
					...
					class="bg-slate-800 border-slate-700 text-white placeholder:text-slate-400"
				/>
				<p class="text-xs text-slate-500">Auto-calculated from project budgets. Edit only to override.</p>
			</div>
