<script lang="ts">
	...
	const emailDeliveryEnabled = data.emailDeliveryEnabled ?? false;
</script>
...
<div class="max-w-3xl mx-auto space-y-8 py-6 px-4">
	...
	<div class="flex items-center gap-3">
		...
		<div>
			...
			<p class="text-sm text-slate-400">Manage your account and feature preferences</p>
		</div>
	</div>

	<div class="rounded-xl border px-4 py-3 {emailDeliveryEnabled ? 'border-emerald-900/60 bg-emerald-950/30' : 'border-amber-900/60 bg-amber-950/30'}">
		<p class="text-xs font-semibold uppercase tracking-wide {emailDeliveryEnabled ? 'text-emerald-300' : 'text-amber-300'}">Current setup status</p>
		{#if emailDeliveryEnabled}
			<p class="mt-1 text-sm text-emerald-100/90">
				Email service is configured. Notification and email-based account flows are enabled for delivery.
			</p>
		{:else}
			<p class="mt-1 text-sm text-amber-100/90">
				Account identity and profile links work now. Email-dependent features, including notification delivery,
				reset links, and verification messages, require email service wiring before they will send.
			</p>
		{/if}
	</div>
	...
	<section class="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
		...
		<div class="divide-y divide-slate-800">
			{#each [
				{ label: 'Approval requests', description: 'Email alert when a task needs your approval' },
				{ label: 'Goal updates', description: 'Email alert when a marketing goal is updated' },
				{ label: 'Expense submissions', description: 'Email alert when an expense is submitted for review' },
			] as pref}
		...
		<p class="px-5 py-3 text-xs border-t border-slate-800 {emailDeliveryEnabled ? 'text-emerald-400/80' : 'text-slate-500'}">
			{#if emailDeliveryEnabled}
				Email notification delivery is enabled.
			{:else}
				Preference storage and delivery are not enabled yet. Email notifications will require email service configuration.
			{/if}
		</p>
	</section>
	...
	<section class="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
		...
		<div class="p-5">
			<p class="text-sm text-slate-400">Theme customisation is planned and does not require email wiring.</p>
		</div>
	</section>
	...
	<section class="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
		...
		<div class="p-5 space-y-3">
			<div class="flex items-center justify-between">
				<div>
					...
					<p class="text-xs text-slate-500">Password management on this page is not enabled yet</p>
				</div>
				<button
					disabled
		...
		<p class="px-5 py-3 text-xs text-slate-500 border-t border-slate-800">
			Security controls are in progress.
			{#if emailDeliveryEnabled}
				 Email-based reset delivery is configured.
			{:else}
				 Any email-based reset flow will require email service wiring.
			{/if}
		</p>
	</section>
