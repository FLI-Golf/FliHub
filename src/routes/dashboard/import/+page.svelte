<script lang="ts">
	...
	import { Upload, CheckCircle2, AlertCircle, X, FileText, Copy, Check } from 'lucide-svelte';
	...
	const TYPES: { id: ImportType; label: string; description: string; color: string }[] = [
		...
		{ id: 'reimbursements', label: 'Reimbursements', description: 'Expense claims with line items',            color: 'bg-orange-950/40 border-orange-700' }
	];
	...
	const SCHEMAS: Record<ImportType, { col: string; required: boolean; example: string; note?: string }[]> = {
		reimbursements: [
			...
			{ col: 'claimStatus',      required: false, example: 'submitted', note: 'draft · submitted · under_review · approved · paid · rejected' },
			{ col: 'claimNotes',       required: false, example: 'Approved by manager' }
		],
		territories: [
...
<div class="space-y-6 max-w-5xl">
	...
	<div>
		...
		<p class="text-muted-foreground mt-1">Paste CSV data to bulk-import vendors, sponsors, pros, territories, or reimbursements</p>
	</div>
